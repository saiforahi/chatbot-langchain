...
def note_down(
    ...
    """
    if not symptoms:
        print("Without symptoms tool called")
    ...
    pre_screening_question_and_answers.append({"Dr Name": doctor_name})
    ...
    direction_link_html = ""
    if doctor_chamber_location:
        latitude = doctor_chamber_location['latitude']
        longitude = doctor_chamber_location['longitude']
        google_maps_direction_link = f"https://www.google.com/maps/dir/?api=1&destination={latitude},{longitude}"
        direction_link_html = f"""<p>I've noted down your preference for {doctor_name}. Below is the direction to the doctor's chamber:</p>
        <div style="text-align: center; margin-top: 20px;">
            <a href="{google_maps_direction_link}" target="_blank" style="display: inline-flex; align-items: center; justify-content: center; text-decoration: none; color: white; background-color: #4CAF50; padding: 10px 20px; border-radius: 5px;">
                <img src="https://cdn.worldvectorlogo.com/logos/google-maps-2020-icon.svg" alt="Google Maps" style="width: 24px; height: 24px; margin-right: 8px;">
                Open Google Maps
            </a>
        </div>
        </br>
        </br>
        <p> Wishing you and your loved ones good health. Please consult with the doctor at your earliest convenience..</p>
        """

    if feedback:
        return f'''
        notebook tool executed, Information is noted down. Has to provide the response in HTML format. Below is the example HTML, I must follow, has to change contents based on the conversation:
        <br>
        `
        {direction_link_html}
        `
        Call Final Answer action.
        '''
    else:
        return "Failed to note down with the given information."
...
def get_notebook_tool(topic_id):
    ...
    wrapped_function = partial(note_down_wrapper, topic_id)

    return StructuredTool.from_function(
        wrapped_function,
        name="notebook_tool",
        description="Useful in case of noting down patient information or to conclude, Wrap-up, Finalize the conversation",
