...
class ChatController(BaseController):
    ...
    def compose_openai_prompt(self, encouragement, instructions):
        ...
        system_message_template: PromptTemplate = PromptTemplate(
            ...
            template="""
        {encouragement}
        Currently, the service is available only in Dhaka, Sydney, London and Bengaluru. It will soon expand to New York, USA; Tokyo, Japan; and Munich, Germany.
        If a user's current or mentioned location is not served lby us, first verify whether it is in Dhaka or in Sydney or in London or in Bengaluru. If it is not, you must inform them that the service is not yet available in their location.
    ...
    def compose_claude_json_agent_prompt(self, encouragement, instructions):
        user_location = (f"The person you are having conversation with is from {self.user_address.get('formatted')}. "
                         f"You know it by information given from system. When responding, please adapt your language to reflect the {self.user_address.get('country')} dialect and cultural nuances. This means using local expressions, and idioms where appropriate, while maintaining clear communication. Adjust your responses to mirror the linguistic characteristics of {self.user_address.get('country')}, enhancing the connection and relatability ") if self.user_address else "You must ask user for their human readable address before calling suggest_nearby_doctors tool."

        system_message_template: PromptTemplate = PromptTemplate(
            input_variables=["tool_names", "tools", "chat_history"],
            partial_variables={
                "encouragement": encouragement,
                "instructions": instructions,
                "language_name": self.llm_lang,
        ...
        **Option 3:** (For response to human, final answer, greetings, direct responses, including errors, limitations or requests for missing information)
        ```json
        {{
            "action": "Final Answer",
            "action_input": "Your final response to human here"
        ...
        *** (reminder to respond in a JSON blob no matter what) ***
        ...
        chatbot_system_message_prompt = HumanMessagePromptTemplate(
            prompt=system_message_template
        )
        ai_system_message_prompt = AIMessagePromptTemplate(
        ...
        ai_reply_message_prompt = AIMessagePromptTemplate(
            prompt=PromptTemplate(
        ...
        messages = [
            chatbot_system_message_prompt,
            # MessagesPlaceholder(variable_name="chat_history"),
            ai_system_message_prompt,
            human_message_template,
            ai_reply_message_prompt
        ]
    ...
    def prepare_prompt(self, encouragement, instructions):
        try:
            prompt = None
            if self.chat_bot.llm.origin == "bedrock":
                prompt = self.compose_claude_json_agent_prompt(
    ...
    async def chat_call(self, message):
        try:
            ...
            agent_executor = AgentExecutor(
                ...
                verbose=False,
                handle_parsing_errors="Call Final Answer action with desired JSON format",
                memory=memory,
            ...
            if type(e) == RateLimitError and e.type == "insufficient_quota":
                socketio.emit(f"agent/{current_user.id}", {"data": {"message": "Assistant cannot reply now"},
                                                           "action": ACTIONS["insufficient_quota"]})
